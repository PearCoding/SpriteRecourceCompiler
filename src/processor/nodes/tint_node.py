...
class TintNode(ProcessingNode):
    ...
    def execute(self, processor, img):
        ...
        if self.mode == ImageFilterMode.Multiply:
            r = source[R].point(lambda i: i * (1-self.opacity) + (self.color[R]/255.0)*i*self.opacity)
            g = source[G].point(lambda i: i * (1-self.opacity) + (self.color[G]/255.0)*i*self.opacity)
            b = source[B].point(lambda i: i * (1-self.opacity) + (self.color[B]/255.0)*i*self.opacity)
